...
export const searchAlternate = async (req: Request, res: Response) => {
    try {
        ...
        // ✅ Cari Property yang memiliki Room dengan harga sesuai
            ...
            where: {
                ...
                rooms: {
                    some: {
                        isAvailable: true,
                        price: {
                            gte: minPriceNum ?? 0,  // ✅ Pastikan minPriceNum dikonversi ke Float
                            lte: maxPriceNum ?? Number.MAX_SAFE_INTEGER, // ✅ Sama untuk maxPriceNum
                        },
                    },
                },
            },
        ...
        if (sort === "price_asc") {
            orderByClause = { rooms: { _min: { price: "asc" } } };  // ✅ Sort dari harga terendah
        } else if (sort === "price_desc") {
            orderByClause = { rooms: { _min: { price: "desc" } } }; // ✅ Sort dari harga tertinggi
        }


        // ✅ Cari harga terendah dengan mempertimbangkan Peak Season
